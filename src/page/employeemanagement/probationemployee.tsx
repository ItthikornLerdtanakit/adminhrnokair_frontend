...
import { FormatDate, RemainingDays } from '../component/functions';

import { Search, Edit } from 'lucide-react';

const Probationemployee = () => {
    useEffect(() => {
        document.title = 'Probation Employee';
        get_database();;
    }, []);
    ...
    const get_database = async () => {
        ...
        const employee = result_employee.filter((item: EmployeeWithDepartment) => item.employee_status === 'probation')
            ...
            <Container fluid className='py-4 content flex-grow-1 margintop'>
                <Card className='shadow-sm' style={{ border: 'none', width: '100%' }}>
                    <Card.Header className='bg-warning form-header pt-4'>
                        <h2>List of Probation Employees</h2>
                        <p>Total number of probation employees: {Employee.length}</p>
                    </Card.Header>
                    <Card.Body>
                        <Row className='mb-3'>
                                ...
                                <tbody>
                                    {FilterEmployees.map((emp) => (
                                        <tr key={emp.employee_id}>
                                            ...
                                            <td className='text-center' style={{ color: Number(RemainingDays(emp.employee_startdate)) < 0 ? 'red' : 'black' }}>{RemainingDays(emp.employee_startdate)} days</td>
