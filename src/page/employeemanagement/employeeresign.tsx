...
const Employeeresign = () => {
    useEffect(() => {
        document.title = 'Employee Resign';
        get_database();
    }, []);
    ...
    const get_database = async () => {
        ...
        const employee = result_employee.filter((item: EmployeeWithDepartment) => item.employee_status === 'resign');
            ...
            <Container fluid className='py-4 content flex-grow-1 margintop'>
                <Card className='shadow-sm' style={{ border: 'none', width: '100%' }}>
                    <Card.Header className='bg-warning form-header pt-4'>
                        ...
                        <p>Total number of resigned employees: {Employee.length}</p>
                    </Card.Header>
                    <Card.Body>
                        <Row className='mb-3'>
                                ...
                                <tbody>
                                    {FilterEmployees.map((emp) => (
                                        <tr key={emp.employee_id}>
                                            <td width={150}><strong>{emp.employee_code}</strong></td>
                                            <td width={200}>{emp.employee_nameen}</td>
                                            <td width={200}>{emp.employee_position}</td>
